...
function removeFirstOccurrences(str, value) {
  const countStr = str.indexOf(value);
  if (countStr < 0) {
    return str;
  }
  return str.slice(0, countStr) + str.slice(countStr + value.length);
}
...
function removeLastOccurrences(str, value) {
  const countStr = str.lastIndexOf(value);
  if (countStr < 0) {
    return str;
  }
  return str.slice(0, countStr) + str.slice(countStr + value.length);
...
function isPalindrome(str) {
  ...
  count /= 2;
  if (newStr.endsWith(newStr.substr(0, count).split('').reverse().join(''))) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  ...
  return sentence.split(' ').reduce((acc, item) => {
    result = item.length > acc.length ? item : acc;
    return result;
  });
}
...
function reverseWords(str) {
  ...
  return arrWord.map((item) => item.split('').reverse().join('')).join(' ');
}
...
function extractNameFromTemplate(value) {
  let arrWord = value.split(' ');
  arrWord.shift();
  arrWord = arrWord.join(' ').split('');
  arrWord.pop();
  return arrWord.join('');
}
    ...
    .map((item) => {
      return item
        ...
        .map((elem) => {
          if (
            elem.charCodeAt() < 65 ||
            (elem.charCodeAt() > 90 && elem.charCodeAt() < 97) ||
            elem.charCodeAt() > 122
          ) {
            return elem;
          }
          let newElem = elem.charCodeAt() + 13;
          if (elem.charCodeAt() < 91) {
            if (newElem > 90) {
              newElem = newElem - 90 + 64;
            }
          } else if (newElem > 122) {
            newElem = newElem - 122 + 96;
          }
          newElem = String.fromCharCode(newElem);
          return newElem;
        })
        .join('');
    })
